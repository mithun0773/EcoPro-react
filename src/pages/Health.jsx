...
const Health = () => {
  ...
  // Extract years and health values (1960–2023)
  const years = Object.keys(healthData).filter(
    (key) => Number(key) >= 1960 && Number(key) <= 2023
  );
  const values = years.map(
    (year) => Number(healthData[year]?.replace(/,/g, "")) || 0
  );
  ...
  const lineData = {
    ...
    datasets: [
      {
        label: `health Growth Trend of ${country}`,
        data: values,
        borderColor: "#4f46e5",
  ...
  const doughnutData = {
    ...
    datasets: [
      {
        label: "Recent health Distribution",
        data: values.slice(-6),
        backgroundColor: [
          "#4f46e5",
          "#3b82f6",
  ...
  return (
    ...
    >
      ...
      <p style={{ color: "#555" }}>
        Explore health growth trends of countries (1960–2023)
      </p>
      ...
      >
        <div style={styles.card}>
          <h3>📦 Bar Chart — health by Year</h3>
          <Bar data={barData} options={chartOptions} />
        </div>

        <div style={styles.card}>
          <h3>📈 Line Chart — health Trend</h3>
          <Line data={lineData} options={chartOptions} />
        </div>

        <div style={styles.card}>
          <h3>🍩 Doughnut — Recent health (2018–2023)</h3>
