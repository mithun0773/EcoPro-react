...
const Gdp = () => {
  ...
  // Extract years and gdp values (1960–2023)
  const years = Object.keys(gdpData).filter(
    (key) => Number(key) >= 1960 && Number(key) <= 2023
  );
  const values = years.map(
    (year) => Number(gdpData[year]?.replace(/,/g, "")) || 0
  );
  ...
  const lineData = {
    ...
    datasets: [
      {
        label: `gdp Growth Trend of ${country}`,
        data: values,
        borderColor: "#4f46e5",
  ...
  return (
    ...
    >
      <h1>🌍 gdp Dashboard</h1>
      <p style={{ color: "#555" }}>
        Explore gdp growth trends of countries (1960–2023)
      </p>
      ...
      >
        <div style={styles.card}>
          <h3>📦 Bar Chart — gdp by Year</h3>
          <Bar data={barData} options={chartOptions} />
        </div>

        <div style={styles.card}>
          <h3>📈 Line Chart — gdp Trend</h3>
          <Line data={lineData} options={chartOptions} />
        </div>

        <div style={styles.card}>
          <h3>🍩 Doughnut — Recent gdp (2018–2023)</h3>
