...
const Agri = () => {
  ...
  // Extract years and agri values (1960–2023)
  const years = Object.keys(agriData).filter(
    (key) => Number(key) >= 1960 && Number(key) <= 2023
  );
  const values = years.map(
    (year) => Number(agriData[year]?.replace(/,/g, "")) || 0
  );
  ...
  const lineData = {
    ...
    datasets: [
      {
        label: `agri Growth Trend of ${country}`,
        data: values,
        borderColor: "#4f46e5",
  ...
  const doughnutData = {
    ...
    datasets: [
      {
        label: "Recent agri Distribution",
        data: values.slice(-6),
        backgroundColor: [
          "#4f46e5",
          "#3b82f6",
  ...
  return (
    ...
    >
      ...
      <p style={{ color: "#555" }}>
        Explore agri growth trends of countries (1960–2023)
      </p>
      ...
      >
        <div style={styles.card}>
          <h3>📦 Bar Chart — agri by Year</h3>
          <Bar data={barData} options={chartOptions} />
        </div>

        <div style={styles.card}>
          <h3>📈 Line Chart — agri Trend</h3>
          <Line data={lineData} options={chartOptions} />
        </div>

        <div style={styles.card}>
          <h3>🍩 Doughnut — Recent agri (2018–2023)</h3>
