...
const Population = () => {
  ...
  // Extract years and population values (1960–2023)
  const years = Object.keys(populationData).filter(
    (key) => Number(key) >= 1960 && Number(key) <= 2023
  );
  const values = years.map(
    (year) => Number(populationData[year]?.replace(/,/g, "")) || 0
  );
  ...
  const lineData = {
    ...
    datasets: [
      {
        label: `Population Growth Trend of ${country}`,
        data: values,
        borderColor: "#4f46e5",
  ...
  const doughnutData = {
    ...
    datasets: [
      {
        label: "Recent Population Distribution",
        data: values.slice(-6),
        backgroundColor: [
          "#4f46e5",
          "#3b82f6",
  ...
  return (
    ...
    >
      ...
      <p style={{ color: "#555" }}>
        Explore population growth trends of countries (1960–2023)
      </p>
      ...
      >
        <div style={styles.card}>
          <h3>📦 Bar Chart — Population by Year</h3>
          <Bar data={barData} options={chartOptions} />
        </div>

        <div style={styles.card}>
          <h3>📈 Line Chart — Population Trend</h3>
          <Line data={lineData} options={chartOptions} />
        </div>

        <div style={styles.card}>
          <h3>🍩 Doughnut — Recent Population (2018–2023)</h3>
